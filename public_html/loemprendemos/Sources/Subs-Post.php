...
function createPost(&$msgOptions, &$topicOptions, &$posterOptions)
{
	...
	//mandar notificaciones de nuevo post si no es nuevo
        /*
        if(!$new_topic){
	        //get members in same topic
		...
		$arreglo_id_member = array();
		foreach($row AS $key => $mensaje_arreglo){
			...
			//get members in same topic
				...
				WHERE id_member= {int:id_member}',
				  array(
				    'id_member' => $mensaje_arreglo,
				  )
			);
			$row2 = $smcFunc['db_fetch_assoc']($request);
		
			//almacenar arreglo
			$arreglo_id_member[] = $mensaje_arreglo;
			//send mail about reply messages
			//sendmail($to, $subject, $message, $from = null, $message_id = null, $send_html = false, $priority = 3, $hotmail_fix = null, $is_private = false)
			//sendmail(''.$row2['email_address'].'', 'Loemprendemos: Nueva Respuesta a tu Mensaje', 'Hola '.$row2['real_name'].'\n\n'.$msgOptions['subject'].'', null, null, false, 3, null, false);
		}
		//sendpm(array recipients, string subject, string message, bool store_outbox = false, array from = current_member, int pm_head = 0)
		
	}

        //fin mandar notificaciones de nuevo post*/
