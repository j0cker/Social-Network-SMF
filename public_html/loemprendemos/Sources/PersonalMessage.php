...
function MessageFolder()
{
	...
	if (isset($_GET['view']))
	{
        ...
        //forza a que sea una conversación
	...
	if (!empty($pms))
	{
		...
		if ($context['display_mode'] == 2)
		{
			$request = $smcFunc['db_query']('', '
				...
				WHERE pm.id_pm_head = {int:id_pm_head}
					AND ((pm.deleted_by_sender = {int:not_deleted})
						OR (pmr.deleted = {int:not_deleted}))
				ORDER BY pm.id_pm',
				array(
					'current_member' => $user_info['id'],
		...
		if ($context['display_mode'] != 0)
		{
			...
			// Seperate query for these bits! messages subject's recent
		...
		// Execute the query! messages conversation
