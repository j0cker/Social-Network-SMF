...
function editBuddies($memID)
{
	...
	global $context, $user_profile, $memberContext, $smcFunc, $sourcedir;
	...
	{
                $url = "".$modSettings["pretty_root_url"]."/enviarDatosConecta.php";

		$postdata = http_build_query(
		    array('id_send' => $_GET['request'],'id_member' => $_GET['u'])
		, '', '&');
		...
		//manda llamar a la función donde se realiza el request
		$result = file_get_contents($url, false, $context);                
 
                redirectexit('action=profile;u=' . (int) $_GET['request']);
        }

        // Adding a buddy?
	if (isset($_GET['add']) && is_numeric($_GET['add']))
	{       //url -> /profile/?area=lists;sa=buddies;u=42;add=44

		//checkSession('get'); -> checa por get las sesiones

            $request = $smcFunc['db_query']('set_character_set', '
	                SELECT buddy_list, buddy_pending, real_name, email_address
	    ...
	    $row = $smcFunc['db_fetch_assoc']($request);
            ...
            $request = $smcFunc['db_query']('set_character_set', '
	                SELECT buddy_pending, real_name, email_address
	                FROM {db_prefix}members
	                WHERE id_member = {int:id_member}',
	                array(
	                    'id_member' => $_GET['u'],
	    ...
	    $row3 = $smcFunc['db_fetch_assoc']($request);

            //si el usuario que confirma solicitud está en lista de pendientes del usuario solicitante entonces son amigos. (de otro forma habría un fallo de seguridad)
            $entra = 0;
            $buddy_list_array_2 = explode(",",$row['buddy_pending']);
		...
		foreach ($buddiesArray as $key => $buddy){
			if ($buddy == (int) $_GET['add'])
				$existe = 1;
                }

                if($existe==0){
                  array_push($buddiesArray,(int) $_GET['add']); 
                  
                } else {
                  redirectexit('action=profile;u=' . (int) $_GET['add']);
                }

		 // Make the changes.
		  $user_profile[$memID]['buddy_list'] = implode(',', $buddiesArray);

                  // updateMemberData no checa si existe o no el usuario (igual lo inserta)
		  updateMemberData($memID, array('buddy_list' => $user_profile[$memID]['buddy_list']));
		
		
		//Los agregamos mutuamente
		
		
	        $row2 = explode(',', $row['buddy_list']);
		foreach ($row2 as $k => $dummy)
			if ($dummy == '')
				unset($row2[$k]);
		...
		// Heh, I'm lazy, do it the easy way...
		foreach ($buddiesArray2 as $key => $buddy){
			if ($buddy == (int) $_GET['u'])
				$existe = 1;
                }

                if($existe==0){
                  array_push($buddiesArray2,(int) $_GET['u']); 
                } else {
                  redirectexit('action=profile;u=' . (int) $_GET['u']);
                }

		// Make the changes.
		$buddiesArray2 = implode(',', $buddiesArray2);
		
                // updateMemberData no checa si existe o no el usuario (igual lo inserta)
		updateMemberData($_GET['add'], array('buddy_list' => $buddiesArray2));

                //quitamos de pendiente

                $row2 = explode(',', $row['buddy_pending']);
		foreach ($row2 as $k => $dummy)
			if ($dummy == '')
				unset($row2[$k]);
		...
		$buddiesArray2 = implode(',', $buddiesArray2);
                ...
                // updateMemberData no checa si existe o no el usuario (igual lo inserta)
		updateMemberData($_GET['add'], array('buddy_pending' => $buddiesArray2));

                //quitamos de pendiente mutuo

                $row2 = explode(',', $row3['buddy_pending']);
		foreach ($row2 as $k => $dummy)
			if ($dummy == '')
				unset($row2[$k]);
		...
		$buddiesArray2 = implode(',', $buddiesArray2);
                ...
                // updateMemberData no checa si existe o no el usuario (igual lo inserta)
		updateMemberData($_GET['u'], array('buddy_pending' => $buddiesArray2));

                //Ahora son amigos (Mandar Emails)
                $subject = ''.$txt['solicitud_conexion_subject2'].'';
                $body = ''.$txt['hello_member'].' '.$row["real_name"].'.'.$txt['solicitud_conexion6'].''.$row3["real_name"].' '.$txt['solicitud_conexion7'].''.$txt['solicitud_conexion8'].''.$modSettings["pretty_root_url"].'/pm/?sa=send;u='.$_GET['u'].';mail=3;send=friends '.$txt['solicitud_conexion9'].''.$modSettings["pretty_root_url"].'/profile/?u='.$_GET['u'].';mail=3;send=profile'.$txt['solicitud_conexion10'].''.$txt['solicitud_conexion5'].''.$txt['nombre_del_smf_general'].'.';

                //sendmail($to, $subject, $message, $from = null, $message_id = null, $send_html = false, $priority = 3, $hotmail_fix = null, $is_private = false)
                sendmail($row["email_address"], $subject, $body, null, null, false, 3, null, false);

                $body = ''.$txt['hello_member'].' '.$row3["real_name"].'.'.$txt['solicitud_conexion6'].''.$row["real_name"].' '.$txt['solicitud_conexion7'].''.$txt['solicitud_conexion8'].''.$modSettings["pretty_root_url"].'/pm/?sa=send;u='.$_GET['add'].' '.$txt['solicitud_conexion9'].''.$modSettings["pretty_root_url"].'/profile/?u='.$_GET['add'].''.$txt['solicitud_conexion10'].''.$txt['solicitud_conexion5'].''.$txt['nombre_del_smf_general'].'.';

                //sendmail($to, $subject, $message, $from = null, $message_id = null, $send_html = false, $priority = 3, $hotmail_fix = null, $is_private = false)
                sendmail($row3["email_address"], $subject, $body, null, null, false, 3, null, false);
	...
	{       //url -> /profile/?area=lists;sa=buddies;u=42;add=44
		...
		$buddiesArray2 = implode(',', $buddiesArray2);
                ...
                // updateMemberData no checa si existe o no el usuario (igual lo inserta)
		updateMemberData($_GET['remove'], array('buddy_list' => $buddiesArray2));
