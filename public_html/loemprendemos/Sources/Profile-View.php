...
/*
	...
	void showPosts(int id_member)
        ...
        void mixWallForum(int id_member)
                // !!!
		...
		else
		{
			$request = $smcFunc['db_query']('', '
				SELECT
					...
					m.subject, m.poster_time, m.approved, m.id_member_poster, m.member_name_poster, m.likes, m.likes_users
				FROM {db_prefix}messages AS m
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		$context['posts'][$counter += $reverse ? -1 : 1] = array(
			...
			'approved' => $row['approved'],
                        ...
                        'member_name_poster' => $row['member_name_poster'],
                        'likes' => $row['likes'],
	...
	global $context, $user_profile, $sourcedir, $smcFunc;
        ...
        $request = $smcFunc['db_query']('', '
				SELECT  m.body, m.smileys_enabled, m.subject, m.poster_time, m.approved, 
                                        ...
                                        m.id_project
				FROM {db_prefix}messages AS m
				WHERE m.id_member = {int:current_member} AND m.id_topic=0 AND m.id_board=0
        ...
        while($row = $smcFunc['db_fetch_assoc']($request)){
          $posts2[$c] = array(
			...
			'approved' => $row['approved'],
                        'poster_time' => $row['poster_time'],
                        'id_member_poster' => $row['id_member_poster'],
                        'name_member_poster' => $row['name_member_poster'],
                        'likes' => $row['likes'],
                        'likes_users' => $row['likes_users'],                        
        ...
        $c=0;
        $total = count($posts2) + count($context['posts']);

        $context['posts'] = array_merge($posts2, $context['posts']);

        usort($context['posts'], "cmp");
  
}

function cmp($a, $b)
{ if($a['poster_time']==$b['poster_time']) return 0;
    return $a['poster_time'] < $b['poster_time']?1:-1;
}
