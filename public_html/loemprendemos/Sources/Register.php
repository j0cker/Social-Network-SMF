...
function Register($reg_errors = array())
{
	...
	global $language, $scripturl, $smcFunc, $sourcedir, $smcFunc, $cur_profile;
        ...
        //El usuario es el mismo que $_POST['user'] = $_POST['customfield']['cust_nombre']; $_POST['customfield']['cust_apelli']

        $_POST['user'] = ''.$_POST['customfield']['cust_nombre'].' '.$_POST['customfield']['cust_apelli'].'';
...
function Register2($verifiedOpenID = false)
{
	...
	elseif ($verifiedOpenID || (!empty($_POST['openid_identifier']) && $_POST['authenticate'] == 'openid'))
	{
        ...
        //Inserta y busca duplicados de usuarios
	...
	else
	{
		...
		redirectexit('action=login2;sa=check;is=newMember;member=' . $memberID, $context['server']['needs_login_fix']);
	}
}
