...
CKEDITOR.dialog.add("base64imageDialog", function(editor){
	...
	function imagePreviewLoad(s) {
                ...
                console.log("imagePreviewLoad: " + s);
		...
		imgPreview.getElement().setHtml('<img class="text-center" style="width: 50px;" src="http://loemprendemos.com/Themes/konusal-bootstrap/images/cargando.gif">');
		...
		i.onload = function() {
			...
			if(orgWidth == null || orgHeight == null) {
				t.setValueOf("tab-properties", "width", "300");
				imgScal = 1;
			...
			try {
				...
				if(p) p.appendChild(this);
                        ...
                        t.getContentElement("tab-properties", "width").disable();
                        t.getContentElement("tab-properties", "height").disable();
                        t.getContentElement("tab-properties", "alt").disable();
		};
				...
				children: [
					{
						type: "checkbox",
						id: "filecheckbox",
						style: "width: 0px; margin-top:5px; display: none;",
    ...
    return {
		...
		onLoad: function(){
			...
			this.getContentElement("tab-properties", "border").getInputElement().on("keyup", function(){ integerValue(this); }, this.getContentElement("tab-properties", "border"));
                        ...
                        var imagePlugin = $(".cke_dialog_body");
			console.log(imagePlugin);
			imagePlugin[imagePlugin.length-1].children[3].children["0"].children["0"].children["0"].children[1].children["0"].children["0"].children[2].style.display="none";
imagePlugin[imagePlugin.length-1].children[3].children["0"].children["0"].children["0"].children[1].children["0"].children["0"].children[1].children["0"].children["0"].children["0"].children["0"].children[2].style.display="none";
                         
                         imagePlugin[imagePlugin.length-1].children[3].children["0"].children["0"].children["0"].children["0"].children["0"].children["0"].children["0"].children["0"].children["0"].children["0"].children["0"].children[1].children["0"].children[1].children["0"].id="uploadImagePostInputCKEditor";

                       
                         imagePlugin[imagePlugin.length-1].children[3].children["0"].children["0"].children["0"].children["0"].children["0"].children["0"].children[1].children["0"].children["0"].children["0"].children["0"].children[1].children["0"].children[1].id= "uploadImagePostSelectCKEditor";

                        $("#uploadImagePostInputCKEditor").children().attr("placeholder","http://example.com/image.png");

                        console.log($("#uploadImagePostInputCKEditor").parent().parent().parent().siblings().children().children()[0]);

                        $("#uploadImagePostSelectCKEditor").children().css("display","none");

                        $("#uploadImagePostSelectCKEditor").children().after('<input onclick="" id="fileImage" type="file" onchange="subirImageCreatePost();" name="file" style="height: 30px; width: 100%; cursor:pointer;">');

                        $("#fileImage").val("");

                        $("#uploadImagePostSelectCKEditor").after('<div style="margin-top: 10px; display: none;" id="cargandoFileImagePost" class="col-md-12 text-center"><center><img class="text-center" style="width: 50px;" src="http://loemprendemos.com/Themes/konusal-bootstrap/images/cargando.gif"></center></div>');
                        $("#uploadImagePostSelectCKEditor").after('<div style="margin-top: 10px;" id="imagePreviewPost" class="col-md-12 text-center"><center></center></div>');

                        this.getContentElement("tab-properties", "width").disable();
		},
		onShow: function(){
		...
		onOk : function(){
			...
			/* Get image source */
			var src = "";
			try { src = CKEDITOR.document.getById(editor.id+"previewimage").$.src; 
                        } catch(e) { 
                          src = ""; 
                          console.log("Fallo obtenidendo SRC");
                          toastr["error"]("ERROR: vuelva a intentarlo.");
                        }
			if(typeof(src) != "string" || src == null || src === "") return;

                        $("#imagePreviewPost").children().children().attr("src","");
                        console.log("src: " + CKEDITOR.document.getById(editor.id+"previewimage").$.src);
			...
			newImg.setAttribute("alt", t.getValueOf("tab-properties", "alt").replace(/^\s+/, "").replace(/\s+$/, ""));
                        newImg.className += "bbc_img";
			var attr = {
				"width" : ["width", "width: 300px;", "integer", 1],
			...
			if(!selectedImg){
                          var newBr = editor.document.createElement("br");
                          editor.insertElement(newBr);
