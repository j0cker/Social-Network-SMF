	...
	echo '
        ...
        <script>
          $( document ).ready(function() {
            console.log( "ready! profile ajax" );
            var comilla = "'; echo "'"; echo '";            

            $.ajax({  	url:   "'.$modSettings["pretty_root_url"].'/enviarDatosProfile.php",
				type:  "POST",
				data:   {id_member_profile:'; echo $context['member']['id']; echo ',id_member:'; echo $context["user"]["id"]; echo '},
				success:  function (response) {
                                  response = JSON.parse(response);
                                  if(response.response=="true"){
                                    $("#tipo_profesion").html(response.tipoProfesion);
                                      if(response.buddyButton=="pendiente"){
                                        $("#buddyButton").html("<button class="+comilla+"btn btn-info"+comilla+" type="+comilla+"button"+comilla+">Solicitud Pendiente de Conexión</button>");
                                        $("#buddyButton").attr("href","");
                                      }
                                      if(response.buddyButton=="confirmar"){
                                        $("#buddyButton").html("<button class="+comilla+"btn btn-danger"+comilla+" type="+comilla+"button"+comilla+">Confirmar Solicitud de Conexión</button>");
                                        $("#buddyButton").attr("href","'; echo $modSettings["pretty_root_url"]; echo '/profile/?area=lists;sa=buddies;u='; echo $context["user"]["id"]; echo ';add='; echo $context['member']['id']; echo ';");
                                      }
                                    }
				},
				error: function (){
          ...
          function publicarEnMuro(){
            console.log( "publicarEnMuro" );
            googleAna("publicarPostProfileDelMuro");
            var comilla = "'; echo "'"; echo '";            

            $.ajax({  	url:   "'.$modSettings["pretty_root_url"].'/publicarEnMuro.php",
				type:  "POST",
				data:   {message:$("#text_message").val(),id_member:'; echo $context["user"]["id"]; echo ',name:"'; echo $context["user"]["name"]; echo '", id_member_profile:'; echo $context['member']['id']; echo ',name_profile:"'; echo $context["member"]["name"]; echo '",},
				success:  function (response) {
                                  response = JSON.parse(response);
                                  if(response.response=="true"){
                                    toastr["success"]("Mensaje Enviado");
                                    window.location = "'.$modSettings["pretty_root_url"].'/index.php?action=profile;u='; echo $context["member"]["id"]; echo '";
                                  } else {
          ...
          function eliminarPost(id_post){
            console.log( "eliminarPostDelMuro" );
            googleAna("eliminarPostProfileDelMuro");
            $.ajax({  	url:   "'.$modSettings["pretty_root_url"].'/eliminarEnMuro.php",
				type:  "POST",
				data:   {idPost:id_post},
				success:  function (response) {
                                  response = JSON.parse(response);
          ...
          function like(id_post, element){
            ...
            $.ajax({  	url:   "'.$modSettings["pretty_root_url"].'/sendLike.php",
				type:  "POST",
				data:   {id_member:'; echo $context["user"]["id"]; echo ', id_member_name:"'; echo $context["user"]["name"]; echo '", id_post:id_post, id_member_profile:'; echo $context["member"]["id"]; echo ', id_member_profile_name:"'; echo $context["member"]["name"]; echo '"},
				success:  function (response) {
                                  response = JSON.parse(response);
                                  if(response.response=="true"){
...
<div class="row">
	<div style="border-radius: 0;" class="cat_bar">
		<h3 class="catbg">
			<span class="ie6_header floatleft">', $txt['summary'], '</span>
		</h3>
	</div>
	<div style="padding: 0;" id="basicinfo" class="col-sm-3">
		<div class="windowbg">
	...
	// What about if we allow email only via the forum??
        /*
	if ($context['member']['show_email'] === 'yes' || $context['member']['show_email'] === 'no_through_forum' || $context['member']['show_email'] === 'yes_permission_override' || $context['user']['is_admin'] || $context['member']['is_buddy'])
		echo '
	...
	if ($context['member']['website']['url'] !== '' && !isset($context['disabled_fields']['website']))
		echo '
					<li><a href="', $context['member']['website']['url'], '" title="' . $context['member']['website']['title'] . '" target="_blank" class="new_win">', ($settings['use_image_buttons'] ? '<img style="width: 22px;" src="' . $settings['images_url'] . '/www_sm.gif" alt="' . $context['member']['website']['title'] . '" />' : $txt['www']), '</a></li>';
	...
	if (!empty($context['can_have_buddy']) && !$context['user']['is_owner'] && $context['member']['id']!=$context['user']['id']){
		echo '
				<br />
                                <a id="buddyButton" onclick="googleAna('.$comillas.'profileButtonRequestPendingAddRemove'.$comillas.');" href="'; echo $modSettings["pretty_root_url"]; echo '/profile/?area=lists;sa=buddies;u=', $context['user']['id'], ';', ($context['member']['is_buddy'] ? 'remove='.$context['member']['id'].'' : 'request='.$context['member']['id'].''), '">
                                  <button style="margin-top: 10px;" type="button" class="btn btn-'.($context['member']['is_buddy'] ? 'danger' : 'success').'"><span class="fa fa-'.($context['member']['is_buddy'] ? 'minus' : 'plus-square').'"></span> ', $txt['buddy_' . ($context['member']['is_buddy'] ? 'remove' : 'add')], '</button>
                                </a>
...
';
	...
	if (!$context['user']['is_owner'] && $context['can_send_pm'])
		echo '
					<a href="', $scripturl, '?action=pm;sa=send;u=', $context['id_member'], '"  onclick="googleAna('.$comillas.'profileButtonSendPM'.$comillas.');">
                                          <button style="margin-top: 10px;"  type="button" class="btn btn-success"><span class="fa fa-envelope"></span> ', $txt['profile_sendpm_short'], '</button>
                                        </a><br />';
        if($context["member"]["id"]==$context["user"]["id"]){
          echo '
          <a href="', $scripturl, '?action=profile;area=lists;sa=buddies;u=', $context['member']['id'], '"  onclick="googleAna('.$comillas.'profileSeeFriends'.$comillas.');">
                                          <button style="margin-top: 10px;"  type="button" class="btn btn-success"><span class="fa fa-user"></span> ', count($context["member"]["buddies"]), ' Conexion(es) y/o Amigo(s)</button>
          </a><br />';
        } else {
          echo '
            <button style="margin-top: 10px;"  type="button" class="btn btn-success"><span class="fa fa-user"></span> ', count($context["member"]["buddies"]), ' Conexion(es) y/o Amigo(s)</button>
	...
	<div style="padding: 0;" id="detailedinfo" class="col-sm-9">
		<div class="windowbg2">
			<span class="topslice"><span></span></span>
			<div class="content">
				<dl>';
        ...
        echo '
        <dt>Tipo de Perfil '.($context['user']['is_owner']? '(<a href="'.$modSettings["pretty_root_url"].'/index.php?action=welcome2">Editar</a>)' : '' ).': </dt><dd id="tipo_profesion"></dd>
        ';
	...
	if ($context['member']['show_email'] == 'yes' || $context['user']['is_admin'] || $context['member']['is_buddy'])
		echo '
					<dt>', $txt['email'], ': </dt>
...
echo "<!--Publicar-->";
//$context['member']['is_buddy']
if (!$context['user']['is_guest'] && ($context['member']['is_buddy'] || $context['user']['id']==$context['member']['id'] || $context['user']['is_admin']))
	{
		echo '
			<a id="quickreply"></a>
			<div class="panel panel-info tborder" id="quickreplybox">
				<div style="color: #000;" class="panel-heading">
				  '.($context['user']['id']==$context['member']['id']? 'Publica en tu Muro' : 'Escribe Algo en su Muro').'
				</div>
				<div class="panel-body" id="quickReplyOptions">

						<form accept-charset="', $context['character_set'], '" name="postmodify" id="postmodify" onsubmit="submitonce(this);" style="margin: 0;">';


			echo '
							<div class="quickReplyContent">
								<textarea id="text_message" cols="600" rows="7" name="message" tabindex="', $context['tabindex']++, '"></textarea>
							</div>
							<div class="righttext padding">
								<input onclick="googleAna('.$comillas.'profilePublishButtonPublish'.$comillas.'); publicarEnMuro();" type="button" name="post" value="', $txt['post'], '" accesskey="s" tabindex="', $context['tabindex']++, '" class="btn btn-success" />';
...
echo "<!--Mensajes del perfil-->";

echo '
		<div class="pagesection">
			<span>', $txt['pages'], ': ', $context['page_index'], '</span>
		</div>';if(count($context['posts'])==0){
                          echo '<p style="color: red;" class="text-center"><div class="alert alert-danger text-center" role="alert">
			...
			<div class="', $post['alternate'] == 0 ? 'windowbg2' : 'windowbg', ' core_posts">
				...
				<div class="content">
					...
					<div class="topic_details">
						<h5><strong>';
                                                if($post['id_project']!=0){
                                                  echo '
                                                  <a href="', $scripturl, '?action=project;request=list">'.$txt['ProjectsList_Head'].'</a> / <a href="', $scripturl, '?action=project;request=see;project='.$post['id_project'].'">', $post['subject'], '</a>';
                                                 } else if($post['topic']!=0) {     
                                                   echo '                                            
                                                  <a href="', $scripturl, '?board=', $post['board']['id'], '.0">', $post['board']['name'], '</a> / <a href="', $scripturl, '?topic=', $post['topic'], '.', $post['start'], '#msg', $post['id'], '">', $post['subject'], '</a>';
                                                   ...
                                                   echo '                                            
                                                  <a href="', $scripturl, '?board=', $post['board']['id'], '.0"><a href="', $scripturl, '?action=profile;u='.$post['id_member_poster'].'">', $post['subject'], '</a>';
                                                 }
                                                echo '
                                                </strong></h5>
					...
					<div style="border-top: 0;" class="list_posts">';
			...
			echo '
					...
					echo '
					  <div style="margin-top: 20px;" class="row text-center">
	                                    <div class="col-md-12 col-xs-12">
	                                      <a onclick="googleAna('.$comillas.'leerMasButtonProfileProjectListNewTimeLineFeed'.$comillas.');" href="http://loemprendemos.com/?action=project;request=see;project='.$post['id_project'].'">
				...
				<div class="floatleft">
					<ul class="reset smalltext quickbuttons">
                                          <li><a><span style="float: left; color: red;" class="fa fa-heart"></span>
 <span>'.$post['likes'].'</span></a></li>
                                        </ul>
                                 </div>';
                        }
				...
				<div class="floatright">
					<ul class="reset smalltext quickbuttons">';
                        ...
                        if(!$context['user']['is_guest']){
                               ...
                               if($existe==0){
                                 echo '
						<li><a onclick="like('.$comillas.''.$post['id'].''.$comillas.',this);"><span class="fa fa-heart-o"></span> <span>Me Gusta</span></a></li>';
                               } else {
                                 echo '
						<li><a onclick="like('.$comillas.''.$post['id'].''.$comillas.',this);"><span style="color: red;" class="fa fa-heart"></span> <span>Ya No Me Gusta</span></a></li>';
                               }
                        }
			...
			if ($post['can_delete']){
				echo '
                        ...
                        } else if($context['user']['is_owner'] || $context['user']['is_admin'] || ($post['id_member_poster']==$context['user']['id'])) {
                                echo '
						<li class="remove_button"><a onclick="eliminarPost('.$comillas.''.$post['id'].''.$comillas.');" onclick="return confirm(\'', $txt['remove_message'], '?\');"><span>', $txt['remove'], '</span></a></li>';
                        }
			if ($post['can_reply'] || $post['can_mark_notify'] || $post['can_delete']){
				echo '
					</ul>
...
function template_edit_options()
{
	...
	foreach ($context['profile_fields'] as $key => $field)
	{
		...
		{  
                   if($cont!=0 || $field['label']=="Texto personal"){
			echo '
						<dt>
