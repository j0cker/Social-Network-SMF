...
function template_main()
{
	...
	while ($message = $context['get_message']())
	{
		...
		echo '
				<div style="border: 0;" class="', $message['approved'] ? ($message['alternate'] == 0 ? 'windowbg' : 'windowbg2') : 'approvebg', '  drop-shadow curled">
		<div class="post_wrapper">

                      <div class="col-sm-3 posbit">';
			// Show avatars, images, etc.?
                        echo '<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">';
			if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
			  { echo '', $message['member']['avatar']['image'], '';}
			else
                          { echo'<img class="avatar" src="'.$settings['images_url'].'/default_avatar.png" alt="" />';}
				echo '
				<button style="padding-left: 10px; padding-right: 10px; font-size: 11px;" onclick="googleAna('.$comillas.'displayTopicProfile'.$comillas.');" type="button" class="btn btn-info btn-sm">', $message['member']['name'], '</button>
                              </a>
                      </div><!--fin posbit-->
		...
		if ($message['can_approve'])
			echo '
									<li onclick="googleAna('.$comillas.'displayTopicMenu', $txt['approve'], ''.$comillas.');" class="approve_button"><a href="', $scripturl, '?action=moderate;area=postmod;sa=approve;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '">', $txt['approve'], '</a></li>';

		// Can they reply? Have they turned on quick reply?
		if ($context['can_quote'] && !empty($options['display_quick_reply']))
			echo '
									<li onclick="googleAna('.$comillas.'displayTopicMenu', $txt['quote'], ''.$comillas.');" class="quote_button"><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '" onclick="return oQuickReply.quote(', $message['id'], ');">', $txt['quote'], '</a></li>';

		// So... quick reply is off, but they *can* reply?
		elseif ($context['can_quote'])
			echo '
									<li onclick="googleAna('.$comillas.'displayTopicMenu', $txt['quote'], ''.$comillas.');" class="quote_button"><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '">', $txt['quote'], '</a></li>';

		// Can the user modify the contents of this post?
		if ($message['can_modify'])
			echo '
									<li onclick="googleAna('.$comillas.'displayTopicMenu', $txt['modify'], ''.$comillas.');" class="modify_button"><a href="', $scripturl, '?action=post;msg=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], '">', $txt['modify'], '</a></li>';

		// How about... even... remove it entirely?!
		if ($message['can_remove'])
			echo '
									<li onclick="googleAna('.$comillas.'displayTopicMenu', $txt['remove'], ''.$comillas.');" class="remove_button"><a href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['remove_message'], '?\');">', $txt['remove'], '</a></li>';

		// What about splitting it off the rest of the topic?
		if ($context['can_split'] && !empty($context['real_num_replies']))
			echo '
									<li onclick="googleAna('.$comillas.'displayTopicMenu', $txt['split'], ''.$comillas.');" class="split_button"><a href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '">', $txt['split'], '</a></li>';

		// Can we restore topics?
		if ($context['can_restore_msg'])
			echo '
									<li onclick="googleAna('.$comillas.'displayTopicMenu', $txt['restore_message'], ''.$comillas.');" class="restore_button"><a href="', $scripturl, '?action=restoretopic;msgs=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '">', $txt['restore_message'], '</a></li>';
				...
				<div class="modal-dialog modal-sm">
				  <div class="modal-content">
					<div class="modal-header">
					  <button onclick="googleAna('.$comillas.'displayTopicProfileCloseHeader'.$comillas.');" type="button" class="close" data-dismiss="modal">&times;</button>
					</div>
		...
		echo '
					...
					<div class="modal-footer">
					  <button onclick="googleAna('.$comillas.'displayTopicProfileCloseFooter'.$comillas.');" type="button" class="btn btn-default" data-dismiss="modal">Close</button>
					</div>
				  </div>
				</div>
			  </div>
		...
		echo '
								<div style="border-top: 0;" class="inner" id="msg_', $message['id'], '"', '>', $message['body'], '
								<br /><br />';
								//si es el primer mensaje del tema pone redes sociales
								if($message['id'] == $context['first_message']){
									$comillas = "'";
									echo '
									<div style="text-align: center;" class="col-sm-12">
									
										<a class="twitter-share-button" href="https://twitter.com/intent/tweet?text=', $context['page_title_html_safe'], '" data-size="large">Tweet</a>
										
										<div style="top: -5px;" class="fb-share-button" data-href="'.$context['linktree'][0]['url'] .'?topic='.$context['current_topic'].'" data-layout="button" data-size="large" data-mobile-iframe="true"><a class="fb-xfbml-parse-ignore" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u='.$context['linktree'][0]['url'].'?topic='.$context['current_topic'].';src=sdkpreparse">Compartir</a></div>
										';
									if(!$context['user']['is_guest']){ 
									//http://'.$_SERVER['HTTP_HOST'].''.$_SERVER['REQUEST_URI'] .'?action=emailuser;sa=sendtopic
									//$context['current_topic']
									//$context['linktree'][3]['url'] -> no creo que sea buena idea por los subforos
									//$context['linktree'][0]['url']
									//$message['href'] -> lo junta con el #msg o de comentario
										echo '
										<a onclick="googleAna('.$comillas.'mailSendButtonDisplayMsgFooter'.$comillas.');" href="'.$context['linktree'][0]['url'] .'?action=emailuser;sa=sendtopic;topic='.$context['current_topic'].'">
											<button style="vertical-align: middle; margin-top: -21px; height: 28px;" class="btn-sm btn-info" type="button"><span class="fa fa-envelope"></span> <b>Enviar</b></button>
										</a>';
									}
									echo '
									</div>';
...
';
                        ...
                        if(!$context['user']['is_guest']){
                               $likes_users = explode(",",$message['likes_users']);
                               $existe = 0;
 ...
 <span style="font-size: 25px;">'.$message['likes'].'</span></a>
                                     </div>
                                     <div class="col-md-6 col-xs-6">';
                               if($existe==0){
                                 echo '
						<a style="float: right; font-size: 25px;" onclick="likeDisplay('.$comillas.''.$message['id'].''.$comillas.','.$comillas.''.$message['member']['id'].''.$comillas.',this, '.$comillas.'display'.$comillas.');"><span class="fa fa-heart-o"></span> <span>Me Gusta</span></a>';
                               } else {
                                 echo '
						<a style="float: right; font-size: 25px;" onclick="likeDisplay('.$comillas.''.$message['id'].''.$comillas.','.$comillas.''.$message['member']['id'].''.$comillas.',this,'.$comillas.'display'.$comillas.');"><span style="color: red;" class="fa fa-heart"></span> <span>Ya No Me Gusta</span></a>';
                               }
                               echo '</div>
                                   </div>
                                 </div>';
...
if ($context['can_reply'] && !empty($options['display_quick_reply']))
		...
		echo '
			...
			echo '
							...
							<div class="righttext padding">
								<input onclick="googleAna('.$comillas.'displayTopicQuikReplyButtonPublish'.$comillas.');" type="submit" name="post" value="', $txt['post'], '" onclick="return submitThisOnce(this);" accesskey="s" tabindex="', $context['tabindex']++, '" class="btn btn-success" />
								<input onclick="googleAna('.$comillas.'displayTopicQuikReplyButtonPreview'.$comillas.');" type="submit" name="preview" value="', $txt['preview'], '" onclick="return submitThisOnce(this);" accesskey="p" tabindex="', $context['tabindex']++, '" class="btn btn-success" />';
	...
	echo '
			<div class="pagesection">
				...
				echo '<div class="nextlinks_bottom">', $context['previous_next'], '</div>
			</div>';
