		...
		if ((!empty($context['smileys']['postform']) || !empty($context['smileys']['popup'])) && !$editor_context['disable_smiley_box'] && $smileyContainer !== null)
		{
			...
			echo '
					...
					sSmileyTemplate: ', JavaScriptEscape('
						<img class="displayTopicIcons" src="%smileySource%" align="bottom" alt="%smileyDescription%" title="%smileyDescription%" id="%smileyId%" />
					'), ',
		...
		if ($context['show_bbc'] && $bbcContainer !== null)
		{
			...
			echo '
					...
					sButtonTemplate: ', JavaScriptEscape('
						<img class="displayTopicBBC" id="%buttonId%" src="%buttonSrc%" align="bottom" width="23" height="22" alt="%buttonDescription%" title="%buttonDescription%" />
					'), ',
...
function template_control_richedit_buttons($editor_id)
{
	...
	echo '
		<input onclick="googleAna('.$comillas.'sendCommentAndTopicPublish'.$comillas.');" type="submit" value="', isset($editor_context['labels']['post_button']) ? $editor_context['labels']['post_button'] : $txt['post'], '" tabindex="', $context['tabindex']++, '" onclick="return submitThisOnce(this);" accesskey="s" class="button_submit" />';

	if ($editor_context['preview_type'])
		echo '
		<input onclick="googleAna('.$comillas.''.$act.'sendCommentAndTopicPreview'.$comillas.');" type="submit" name="preview" value="', isset($editor_context['labels']['preview_button']) ? $editor_context['labels']['preview_button'] : $txt['preview'], '" tabindex="', $context['tabindex']++, '" onclick="', $editor_context['preview_type'] == 2 ? 'return event.ctrlKey || previewPost();' : 'return submitThisOnce(this);', '" accesskey="p" class="button_submit" />';
