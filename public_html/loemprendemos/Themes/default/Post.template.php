...
function template_main()
{
	...
	echo '
						...
						<dd onclick="googleAna('.$comillas.'sendCommentAndTopicIconSelect'.$comillas.');">
...
<script src="'.$modSettings["pretty_root_url"].'/ckeditor/ckeditor.js"></script>
<script src="'.$modSettings["pretty_root_url"].'/ckeditor/samples/js/sample.js"></script>
<link rel="stylesheet" href="'.$modSettings["pretty_root_url"].'/ckeditor/samples/css/samples.css">
<link rel="stylesheet" href="'.$modSettings["pretty_root_url"].'/ckeditor/samples/toolbarconfigurator/lib/codemirror/neo.css">
<script>
function bbcEditorTrans(element){

	CKEDITOR.tools.callFunction(2,event);
	$("#message").html(window.bbcode);
}


function subirImageCreatePost(){
  ga("send", "event", "Subir Imágen Create Post", "click", "Subir Imágen Create Post");
  $("#secondStep").css("display","");
  $("#cargandoFileImagePost").css("display","");
  ...
  { toastr["warning"]("Error, archivo no soportado. Suba otro tipo de archivo");
	$("#cargandoFileImagePost").css("display","none");
	return; 
  }
  var fd = new FormData();
  fd.append("fileImage", oFile);
  console.log(fd);
  $.ajax({  	url:   "'.$modSettings["pretty_root_url"].'/subirImagenes.php",
		type:  "POST",
		data:   fd,
		processData: false,
		contentType: false,
		success:  function (response) {
			...
			if(response.indexOf("ERROR")=="-1"){
			  $("#uploadImagePostInputCKEditor").children().val("'.$modSettings["pretty_root_url"].'/bibliotecaImages/"+response+"").trigger("change");
			  $("#uploadImagePostInputCKEditor").children().focus();
			 $("#uploadImagePostInputCKEditor").parent().parent().parent().siblings().children().children()[0].checked = true;
			 $("#imagePreviewPost").children().html("<img style='.$comillas.'width: 50px;'.$comillas.' width='.$comillas.'50'.$comillas.' src='.$comillas.''.$modSettings["pretty_root_url"].'/bibliotecaImages/"+response+"'.$comillas.' />");
		...
		error: function (){
			...
			<div style="padding-left: 0; padding-right: 0;" class="grid-width-100">
				<div id="editor">
					Escribe Aqui
				</div>
			</div>
<button onclick="bbcEditorTrans(this);" type="button" class="btn btn-primary">Enviar</button>
		</div>
...
';
	...
	if (!empty($settings['additional_options_collapsable']))
		echo '
					<div onclick="googleAna('.$comillas.'sendCommentAndTopicExpandAttachment'.$comillas.');" id="postAdditionalOptionsHeader">
