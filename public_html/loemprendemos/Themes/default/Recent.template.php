...
function template_main()
{
	...
	foreach ($context['posts'] as $post)
	{
                ...
                $imagen = extraer_imagen2($post);
                $width = 300;

                if(!empty($imagen)){
                  if(!empty($imagen[1][0])){
                     $imagen = ''.$imagen[1][0].'.'.$imagen[2][0].'';
                  } 
                }

		echo '
			<div style="'.($post['alternate'] == 0 ? 'border: 1px solid transparent; border-color: #bce8f1; color: #31708f;' : 'border: 1px solid transparent; border-color: #faebcc; color: #31708f; ' ).'" class="', $post['alternate'] == 0 ? 'windowbg' : 'windowbg2', ' core_posts">
				...
				<div class="content">
					...
					<div style="border: 0;" class="list_posts">';
                                        if(!empty($imagen)){
                                          echo '
                                          '.(!empty($imagen[1][0])? "<center><img width=$comilla$width$comilla src=$comilla$imagen$comilla /></center><br />" : "").'';
                                        }
                                        echo '',csubstr(strip_tags_content($post['message']),0,268) , '.....
                                        </div>
				</div>
                                
                                        <div style="margin-top: 20px;" class="text-center row">
                                          <a href="'.$post['href'].'">
                                            <button class="text-center btn-lg btn-primary" type="button">Leer Más</button>
                                          </a>
                                        </div>';
						...
						<tr>
							...
							<td class="subject ', $color_class2, ' windowbg2">
								<div>
									...
									<p>
										', $txt['started_by'], ' <strong>', $topic['first_post']['member']['name'], '</strong>
										', $txt['in'], ' <em>', $topic['board']['name'], '</em>
							...
							<td class="', $color_class2, ' lastpost windowbg2">
								...
								', $txt['by'], ' ', $topic['last_post']['member']['name'], '
							</td>';
...
function extraer_imagen2($texto) {
  
  $matches = "";
  if(isset($texto)){
    $first2_post_msg = $texto;
    $first_post__msg = $first2_post_msg['message'];
